...
const Collection = ({ data, collectionType, emptyStateSubtext, emptyTitle, limit, page, totalPages = 0, urlParamName }: CollectionProps) => {
  return (
    <>
      {data.length > 0 ? (
        <div className='flex flex-col items-center gap-10'>
          <ul className='grid w-full grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3 xl:gap-10'>
            {data.map((event: IEvent) => {
              ...
              return <li key={event._id} className='flex justify-center'>
        ...
        <div className='flex-center wrapper min-h-[200px] w-full flex-col gap-3 rounded-[14px] bg-grey-50 py-28 text-center'>
          <h3 className='p-bold-20 h5-bold'>{emptyTitle}</h3>
